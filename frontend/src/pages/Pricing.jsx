  ...
  return (
    <div style={{ minHeight: '100vh', background: '#070707', color: '#f0ece4' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', padding: '100px 24px 80px' }}>
        <div style={{ fontFamily: 'Montserrat, sans-serif', fontSize: '10px', color: '#e8a020', letterSpacing: '0.2em', textTransform: 'uppercase', fontWeight: '700', marginBottom: '24px' }}>
          Simple Pricing
        </div>
        <h1 style={{ fontFamily: 'Playfair Display, serif', fontSize: 'clamp(42px, 6vw, 80px)', fontWeight: '800', letterSpacing: '-0.03em', lineHeight: '0.95', marginBottom: '24px' }}>
          Start free.<br />
          <span style={{ fontStyle: 'italic', color: '#e8a020' }}>Scale as you grow.</span>
        </h1>
        <p style={{ fontFamily: 'Montserrat, sans-serif', fontSize: '15px', color: '#444', maxWidth: '480px', margin: '0 auto', lineHeight: '1.8', fontWeight: '400' }}>
          No hidden fees. No long-term contracts. Cancel anytime. Start free and upgrade when you're ready.
        </p>
      </div>
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: '1200px', margin: '0 auto', padding: '0 24px 120px', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px' }}>
        {plans.map((plan, i) => (
          <div key={i} className={`plan-card ${plan.popular ? 'popular' : ''}`} style={{ animationDelay: `${i * 0.1}s` }}>
            ...
            {/* Plan name */}
            <div>
              <div style={{ fontFamily: 'Montserrat, sans-serif', fontSize: '10px', color: plan.color, letterSpacing: '0.2em', textTransform: 'uppercase', fontWeight: '700', marginBottom: '8px' }}>
                {plan.name}
              </div>
              <div style={{ fontFamily: 'Playfair Display, serif', fontSize: '42px', fontWeight: '800', color: '#f0ece4', letterSpacing: '-0.03em', lineHeight: '1' }}>
                {plan.price}
              </div>
              <div style={{ fontFamily: 'Montserrat, sans-serif', fontSize: '11px', color: '#333', letterSpacing: '0.1em', marginTop: '4px', fontWeight: '500' }}>
                {plan.period}
              </div>
              <div style={{ fontFamily: 'Montserrat, sans-serif', fontSize: '13px', color: '#444', marginTop: '12px', fontWeight: '400', lineHeight: '1.5' }}>
                {plan.desc}
              </div>
            </div>
            ...
            <button
              ...
              style={{
                background: plan.popular ? '#e8a020' : 'transparent',
                color: plan.popular ? '#070707' : plan.color,
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: '700px', margin: '0 auto', padding: '0 24px 120px' }}>
        <h2 style={{ fontFamily: 'Playfair Display, serif', fontSize: '36px', fontWeight: '800', letterSpacing: '-0.02em', textAlign: 'center', marginBottom: '60px' }}>
          Common questions
        </h2>
        {[
